...
14. Use `Founder launch post`, `Launch reply`, `Scorecard post`, `Warm referral`, `Partner intro`, `Copy warm handoff`, `Referral partner`, `Intro builder`, `One-sheet`, `Decision quiz`, `Value calculator`, `Email a question`, `Copy buyer packet`, or `Community reply` for organic launch announcements, post comments or replies, public posts, warm referrals, partner introductions, prospect-specific warm handoffs, generated warm-intro drafts, printable handoffs, buyer routing, buying-math conversations, hesitant lead questions, forwardable offer summaries, and relevant community discussions where posting is allowed. Use the public conversion checklist when an educational first step fits the conversation, use the Google Business Profile checklist when local search, maps visibility, reviews, photos, or profile conversion is the topic, use the public resource directory when one link should show all free choices, use the referral partner page or its warm intro builder when an accountant, web provider, consultant, chamber contact, or local connector needs a scorecard-first handoff, use the dashboard's `Copy launch post` for the owner's own profile or a permitted launch thread, use `Copy launch reply` when someone engages with that launch post, use the dashboard's `Copy partner intro` when a warm connector needs ready text to introduce the partner page, use the row-level `Copy warm handoff` when that connector or conversation already points to a specific prospect, use the one-sheet when someone needs the short printable version, use the decision quiz when a warm owner is deciding between free scorecard, sample report, fit call, or paid audit, use the value calculator when the owner sees a possible leak but wants to size whether the audit is worth buying, use the homepage email action when the owner has a scope question before booking or buying, and use the homepage buyer packet when someone needs one paste-ready summary to forward internally.
